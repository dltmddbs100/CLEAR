...
directory_path = '/mnt/raid6/dltmddbs100/ckpt/EMNLP25/cross/gte-1.5B-inst/*/*=ours*'
model_files =  glob(directory_path)

# Create global variables for each model file
globals_prefix = "gte_Qwen2_1_5B_instruct_finetuned"
for idx, model_file in enumerate(model_files, start=1):
    globals()[f"{globals_prefix}{idx}"] = ModelMeta(
    loader=partial(  # type: ignore
        instruct_wrapper,
...
directory_path = '../../../ckpt/mgte-base*'
model_files =  glob(directory_path)

# Create global variables for each model file
globals_prefix = "gte_multilingual_base_finetuned"
for idx, model_file in enumerate(model_files, start=1):
    globals()[f"{globals_prefix}{idx}"] = ModelMeta(
    loader=partial(  # type: ignore
        sentence_transformers_loader,
