...
class AbsTaskRetrieval(AbsTask):
    ...
    def _evaluate_subset(
        ...
        ndcg, recall = retriever.evaluate(
            relevant_docs,
            results,
            retriever.k_values,
            ignore_identical_ids=self.ignore_identical_ids,
        ...
        scores = {
            **{f"ndcg_at_{k.split('@')[1]}": v*100 for (k, v) in ndcg.items()},
            # **{f"map_at_{k.split('@')[1]}": v for (k, v) in _map.items()},
            **{f"recall_at_{k.split('@')[1]}": v*100 for (k, v) in recall.items()},
            # **{f"precision_at_{k.split('@')[1]}": v for (k, v) in precision.items()},
            **{f"mrr_at_{k.split('@')[1]}": v*100 for (k, v) in mrr.items()},
            # **{
