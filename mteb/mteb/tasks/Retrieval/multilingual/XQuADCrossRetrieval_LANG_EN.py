...
class XQuADCrossRetrieval_LANG_EN(MultilingualTask, AbsTaskRetrieval):
    metadata = TaskMetadata(
        name="XQuADCrossRetrieval_LANG_EN",
        dataset={
            "path": "google/xquad",
...
@inproceedings{
    ...
    def load_data(self, **kwargs):
        ...
        for lang in self.hf_subsets:
            # Load dataset for questions in the specified language
            question_data = datasets.load_dataset(
                name=f"xquad.{question_language}", **self.metadata_dict["dataset"]
            )[split]
            # Filter out items with no answers
            question_data = question_data.filter(lambda x: x["answers"]["text"] != "")

            # Create a map of question IDs to question texts from the specified language
            question_map = {row["id"]: row["question"] for row in question_data}
            ...
            for row in data:
                ...
                # Use the question from the specified language dataset
                if query_id in question_map:
                    queries[lang][split][query_id] = question_map[query_id]
    ...
    # #VERSION3 (번역을 통한~)
    # def load_data(self, question_language="en", **kwargs):
    #     if self.data_loaded:
    #         return
    ...
    #     langg=self.hf_subsets[0]

    #     # Load dataset for questions in the specified language
    #     question_data = datasets.load_dataset(
    #         'json',data_files=f"/mnt/raid6/hst/ensemble/xquad_translate/gpt/xquad_{langg}_en_translated.json")
        
    #     question_data=question_data['train']['data']
    #     # print(question_data)
        
    #     # Filter out items with no answers
    #     # question_data = question_data.filter(lambda x: x["answers"]["text"] != "")
    #     question_data = [item for item in question_data if item["answers"]["text"]]
