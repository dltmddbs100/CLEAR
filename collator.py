...
class CrossDataCollator:
    ...
    use_dynamic_padding: bool = field(default=True)
        ...
        for column_name in column_names:
            ...
            if self.use_dynamic_padding:
                tokenized = self.tokenizer(
                    [row[column_name] if type(row[column_name]) == str else row[column_name][0] for row in features],
                    max_length = 512,
                    # max_length = 1024, # jina-v3
    ...
    def maybe_warn_about_column_order(self, column_names: list[str]) -> None:
        ...
        for column_name, expected_idx in column_name_to_expected_idx.items():
            if column_name in column_names and column_names.index(column_name) != expected_idx:
                if column_name in ("en_question", "paragraphs", "neg_1", "neg_2", "neg_3", "neg_4", "neg_5", "lang_question", 'neg_query_1','neg_query_2','neg_query_3','neg_query_4','neg_query_5'):
                    proposed_fix_columns = ["en_question", "paragraphs", "neg_1", "neg_2", "neg_3", "neg_4", "neg_5", "lang_question", 'neg_query_1','neg_query_2','neg_query_3','neg_query_4','neg_query_5']
